...
void asPredictorProjCordex::ListFiles(asTimeArray& timeArray) {
    ...
    if (nbFiles == 0) {
        throw exception(asStrF(_("No CORDEX file found for this pattern : %s."), m_fileNamePattern));
    }
    ...
    for (int i = 0; i < listFiles.Count(); ++i) {
        ...
        if (!reDates.Matches(listFiles.Item(i))) {
            throw exception(asStrF(_("The dates sequence was not found in the CORDEX file name : %s."), listFiles.Item(i)));
        }
