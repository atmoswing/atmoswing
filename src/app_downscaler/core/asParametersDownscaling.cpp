    ...
    if (nodeParamBlock->GetName() == "analogs_number") {
      ...
      while (nodeParam) {
        if (nodeParam->GetName() == "preload") {
          ...
          if (!SetPredictorProjDatasetId(iStep, iPtor, asFileParametersDownscaling::GetString(nodeParam))) return false;
        } else if (nodeParam->GetName() == "proj_data_id") {
          if (!SetPredictorProjDataId(iStep, iPtor, asFileParametersDownscaling::GetString(nodeParam))) return false;
        } else if (nodeParam->GetName() == "archive_dataset_id") {
          if (!SetPredictorDatasetId(iStep, iPtor, asFileParametersDownscaling::GetString(nodeParam))) return false;
        } else if (nodeParam->GetName() == "archive_data_id") {
          if (!SetPredictorDataId(iStep, iPtor, asFileParametersDownscaling::GetString(nodeParam))) return false;
        } else if (nodeParam->GetName() == "level") {
          if (!SetPredictorLevel(iStep, iPtor, asFileParametersDownscaling::GetFloat(nodeParam))) return false;
        } else if (nodeParam->GetName() == "time") {
          if (!SetPredictorHour(iStep, iPtor, asFileParametersDownscaling::GetDouble(nodeParam))) return false;
        } else if (nodeParam->GetName() == "members") {
          if (!SetPredictorMembersNb(iStep, iPtor, asFileParametersDownscaling::GetInt(nodeParam))) return false;
          ...
          while (nodeWindow) {
            if (nodeWindow->GetName() == "grid_type") {
              ...
              if (!SetPredictorXmin(iStep, iPtor, asFileParametersDownscaling::GetDouble(nodeWindow))) return false;
            } else if (nodeWindow->GetName() == "x_points_nb") {
              if (!SetPredictorXptsnb(iStep, iPtor, asFileParametersDownscaling::GetInt(nodeWindow))) return false;
            } else if (nodeWindow->GetName() == "x_step") {
              if (!SetPredictorXstep(iStep, iPtor, asFileParametersDownscaling::GetDouble(nodeWindow))) return false;
            } else if (nodeWindow->GetName() == "y_min") {
              if (!SetPredictorYmin(iStep, iPtor, asFileParametersDownscaling::GetDouble(nodeWindow))) return false;
            } else if (nodeWindow->GetName() == "y_points_nb") {
              if (!SetPredictorYptsnb(iStep, iPtor, asFileParametersDownscaling::GetInt(nodeWindow))) return false;
            } else if (nodeWindow->GetName() == "y_step") {
              if (!SetPredictorYstep(iStep, iPtor, asFileParametersDownscaling::GetDouble(nodeWindow))) return false;
            } else {
          ...
          if (!SetPredictorCriteria(iStep, iPtor, asFileParametersDownscaling::GetString(nodeParam))) return false;
        } else if (nodeParam->GetName() == "weight") {
          if (!SetPredictorWeight(iStep, iPtor, asFileParametersDownscaling::GetFloat(nodeParam))) return false;
        } else {
          fileParams.UnknownNode(nodeParam);
        }
        nodeParam = nodeParam->GetNext();
    ...
    if (nodePreprocess->GetName() == "preprocessing_method") {
      ...
      while (nodeParamPreprocess) {
        if (nodeParamPreprocess->GetName() == "proj_dataset_id") {
          if (!SetPreprocessProjDatasetId(iStep, iPtor, iPre, fileParams.GetString(nodeParamPreprocess))) return false;
        } else if (nodeParamPreprocess->GetName() == "proj_data_id") {
          if (!SetPreprocessProjDataId(iStep, iPtor, iPre, fileParams.GetString(nodeParamPreprocess))) return false;
    ...
    for (int j = 0; j < GetPredictorsNb(i); j++) {
      if (NeedsPreprocessing(i, j)) {
        ...
        if (GetPredictorProjDatasetId(i, j).IsEmpty()) {
          wxLogError(_("The projection dataset (step %d, predictor %d) was not provided in the parameters file."), i,
                     j);
          return false;
        }
