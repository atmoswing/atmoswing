...
float asTotalScoreRankHistogramReliability::Assess(const a1f& targetDates, const a1f& scores,
    ...
    float delta = 0;
    float delta_rel = float(scores.size() * (_ranksNb - 1)) / float(_ranksNb);
    for (int i = 0; i < _ranksNb; i++) {
        delta += pow(float(histogram[i]) - float(scores.size()) / float(_ranksNb), 2.0f);
    }
...
float asTotalScoreRankHistogramReliability::AssessOnBootstrap(a1f& histogramPercent, int scoresSize) const {
    ...
    // systems for a scalar variable. Q. J. R. Meteorol. Soc. 131, p. 2131-2150
    float delta = 0;
    float delta_rel = float(scoresSize * (_ranksNb - 1)) / float(_ranksNb);
    for (int i = 0; i < _ranksNb; i++) {
        delta += pow(float(histogramReal[i]) - float(scoresSize) / float(_ranksNb), 2.0f);
    }
