	...
	for (int i_step = 0; i_step<GetStepsNb(); i_step++)
	{
		for (int i_ptor = 0; i_ptor<GetPredictorsNb(i_step); i_ptor++)
		{
			...
			if (NeedsPreloading(i_step, i_ptor))
			{
				...
				if (!IsPredictorXptsnbLocked(i_step, i_ptor))
				{
					int Xbaseptsnb = std::abs(GetPredictorXminUpperLimit(i_step, i_ptor) - GetPredictorXminLowerLimit(i_step, i_ptor)) / GetPredictorXstep(i_step, i_ptor);
				...
				if (!IsPredictorYptsnbLocked(i_step, i_ptor))
				{
					int Ybaseptsnb = std::abs(GetPredictorYminUpperLimit(i_step, i_ptor) - GetPredictorYminLowerLimit(i_step, i_ptor)) / GetPredictorYstep(i_step, i_ptor);
...
void asParametersOptimization::InitRandomValues()
{
    ...
    for (int i=0; i<GetStepsNb(); i++)
    {
        ...
        for (int j=0; j<GetPredictorsNb(i); j++)
        {
            ...
            if (NeedsPreprocessing(i,j) && GetPreprocessMethod(i,j).IsSameAs("Gradients") && GetPredictorCriteria(i, j).IsSameAs("S1"))
            {
