...
bool asParametersOptimizationGAs::IsParamLocked(int index) {
  ...
  wxASSERT_MSG(
      ...
      wxString::Format("The counter (%d) did not match the number of parameters (%d).", counter, m_allParametersCount));
  ...
  do {
    if (!IsParamLocked(counter)) {
      if (asRandom(0.0, 1.0) < probability) {
        double newVal =
            asRandom(GetParameterLowerLimit(counter), GetParameterUpperLimit(counter), GetParameterIteration(counter));
