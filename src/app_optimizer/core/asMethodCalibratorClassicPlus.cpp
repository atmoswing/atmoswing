...
bool asMethodCalibratorClassicPlus::Calibrate(asParametersCalibration &params)
{
    ...
    for (unsigned int i_stat = 0; i_stat < stationsId.size(); i_stat++) {
        ...
        for (int i_step = 0; i_step < stepsNb; i_step++) {
            ...
            if (predictorXminIteration != 0 || predictorXptsnbIteration != 0 || predictorYminIteration != 0 ||
                predictorYptsnbIteration != 0) {
                ...
                for (int i_x = 0; i_x <= ((predictorXminUpperLimit - predictorXminLowerLimit) /
                                          (predictorXminIteration * stepsLonPertinenceMap)); i_x++) {
                    for (int i_y = 0; i_y <= ((predictorYminUpperLimit - predictorYminLowerLimit) /
                                              (predictorYminIteration * stepsLatPertinenceMap)); i_y++) {
                    ...
                    if (proceedSequentially) {
                        ...
                        for (int sub_step = i_step; sub_step < stepsNb; sub_step++) {
                            ...
                            if (sub_step == 0) {
                                ...
                                if (!GetAnalogsSubDates(anaDates, m_parametersTemp[i_param], anaDatesPreviousSubRuns,
                                                        sub_step, containsNaNs))
                                    return false;
                            }
                            if (containsNaNs) {
                                continueLoop = false;
                        ...
                        if (continueLoop) {
                            if (!GetAnalogsValues(anaValues, m_parametersTemp[i_param], anaDates, stepsNb - 1))
                                return false;
                            if (!GetAnalogsForecastScores(anaScores, m_parametersTemp[i_param], anaValues, stepsNb - 1))
                                return false;
                ...
                asLogMessageImportant(wxString::Format(_("Best point on relevance map: %.2f lat, %.2f lon"),
                                                       m_parameters[m_parameters.size() - 1].GetPredictorYmin(i_step,
                                                                                                              0),
                                                       m_parameters[m_parameters.size() - 1].GetPredictorXmin(i_step,
                                                                                                              0)));

                // Resize domain
                asLogState(
                        wxString::Format(_("Calibration: resize the spatial domain for every predictor (station %s)."),
                                         GetPredictandStationIdsList(stationId)));
                        ...
                        if (proceedSequentially) {
                            ...
                            for (int sub_step = i_step; sub_step < stepsNb; sub_step++) {
                                ...
                                if (sub_step == 0) {
                                    ...
                                    if (!GetAnalogsSubDates(anaDates, params, anaDatesPreviousSubRuns, sub_step,
                                                            containsNaNs))
                                        return false;
                                }
                                if (containsNaNs) {
                ...
                asLogMessageImportant(
                        wxString::Format(_("Time to process the first resizing procedure: %ldms"), swEnlarge.Time()));

                // Resize domain
                asLogState(wxString::Format(
                        _("Calibration: reshape again the spatial domain for every predictor (station %s)."),
                        GetPredictandStationIdsList(stationId)));
                    ...
                    for (int i_resizing = 0; i_resizing < 22; i_resizing++) {
                        ...
                        for (int i_ptor = 0; i_ptor < ptorsNb; i_ptor++) {
                            switch (i_resizing) {
                                case 0:
                                    // Enlarge all
                                    yptsnbtmp = params.GetPredictorYptsnb(i_step, i_ptor) +
                                                2 * multipleFactor * predictorYptsnbIteration;
                                    yptsnbtmp = wxMax(wxMin(yptsnbtmp, predictorYptsnbUpperLimit),
                                                      predictorYptsnbLowerLimit);
                                    xptsnbtmp = params.GetPredictorXptsnb(i_step, i_ptor) +
                                                2 * multipleFactor * predictorXptsnbIteration;
                                    xptsnbtmp = wxMax(wxMin(xptsnbtmp, predictorXptsnbUpperLimit),
                                                      predictorXptsnbLowerLimit);
                                    params.SetPredictorYptsnb(i_step, i_ptor, yptsnbtmp);
                                    params.SetPredictorXptsnb(i_step, i_ptor, xptsnbtmp);
                                    ytmp = params.GetPredictorYmin(i_step, i_ptor) -
                                           multipleFactor * predictorYminIteration;
                                    ytmp = wxMax(wxMin(ytmp, predictorYminUpperLimit), predictorYminLowerLimit);
                                ...
                                case 2:
                                    // Reduce top
                                    yptsnbtmp = params.GetPredictorYptsnb(i_step, i_ptor) -
                                                multipleFactor * predictorYptsnbIteration;
                                    yptsnbtmp = wxMax(wxMin(yptsnbtmp, predictorYptsnbUpperLimit),
                                                      predictorYptsnbLowerLimit);
                                    params.SetPredictorYptsnb(i_step, i_ptor, yptsnbtmp);
                                    break;
                                case 3:
                                    // Reduce right
                                    xptsnbtmp = params.GetPredictorXptsnb(i_step, i_ptor) -
                                                multipleFactor * predictorXptsnbIteration;
                                    xptsnbtmp = wxMax(wxMin(xptsnbtmp, predictorXptsnbUpperLimit),
                                                      predictorXptsnbLowerLimit);
                                    params.SetPredictorXptsnb(i_step, i_ptor, xptsnbtmp);
                                    break;
                        ...
                        if (KeepIfBetter(params, anaScoreFinal)) {
                            asLogMessageImportant(
                                    wxString::Format("Improved spatial window size and position (move %d, factor %d)",
                                                     i_resizing, multipleFactor));
                            i_resizing = 0;
                            multipleFactor = 1;
                        }
                    }
                ...
                asLogMessageImportant(
                        wxString::Format(_("Time to process the second resizing procedure: %ldms"), swResize.Time()));
