        ...
        if (nodeParam->GetName() == "preload") {
          ...
          while (nodeWindow) {
            if (nodeWindow->GetName() == "grid_type") {
              ...
              if (!SetPredictorXminVector(iStep, iPtor, fileParams.GetVectorDouble(nodeWindow))) return false;
            } else if (nodeWindow->GetName() == "x_points_nb") {
              if (!SetPredictorXptsnbVector(iStep, iPtor, fileParams.GetVectorInt(nodeWindow))) return false;
              ...
              if (!SetPredictorYminVector(iStep, iPtor, fileParams.GetVectorDouble(nodeWindow))) return false;
            } else if (nodeWindow->GetName() == "y_points_nb") {
              if (!SetPredictorYptsnbVector(iStep, iPtor, fileParams.GetVectorInt(nodeWindow))) return false;
    ...
    if (nodePreprocess->GetName() == "preprocessing_method") {
      ...
      while (nodeParamPre) {
        if (nodeParamPre->GetName() == "dataset_id") {
          ...
          if (!SetPreprocessDataIdVector(iStep, iPtor, iPre, fileParams.GetVectorString(nodeParamPre))) return false;
          if (!SetPreprocessDataId(iStep, iPtor, iPre, fileParams.GetVectorString(nodeParamPre)[0])) return false;
        } else if (nodeParamPre->GetName() == "level") {
          if (!SetPreprocessLevelVector(iStep, iPtor, iPre, fileParams.GetVectorFloat(nodeParamPre))) return false;
          if (!SetPreprocessLevel(iStep, iPtor, iPre, fileParams.GetVectorFloat(nodeParamPre)[0])) return false;
        } else if (nodeParamPre->GetName() == "time") {
          if (!SetPreprocessHourVector(iStep, iPtor, iPre, fileParams.GetVectorDouble(nodeParamPre))) return false;
          if (!SetPreprocessHour(iStep, iPtor, iPre, fileParams.GetVectorDouble(nodeParamPre)[0])) return false;
...
double asParametersCalibration::GetPredictorHoursLowerLimit(int iStep, int iPtor) const {
  ...
  wxASSERT(lastrow >= 0);
  double val =
      asMinArray(&m_stepsVect[iStep].predictors[iPtor].hours[0], &m_stepsVect[iStep].predictors[iPtor].hours[lastrow]);
  return val;
}
...
double asParametersCalibration::GetPredictorHoursUpperLimit(int iStep, int iPtor) const {
  ...
  wxASSERT(lastrow >= 0);
  double val =
      asMaxArray(&m_stepsVect[iStep].predictors[iPtor].hours[0], &m_stepsVect[iStep].predictors[iPtor].hours[lastrow]);
  return val;
}
