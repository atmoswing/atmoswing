  ...
  while (true) {
    ...
    while (m_iterator < m_paramsNb) {
      ...
      if (nextParams) {
        ...
        auto *thread = new asThreadGeneticAlgorithms(this, nextParams, &m_scoresCalib[m_iterator], &m_scoreClimatology);
#ifdef USE_CUDA
        if (method == asCUDA) {
  ...
  if (m_allowElitismForTheBest && !m_parametersTemp.empty()) {
    switch (m_scoreOrder) {
      case (Asc): {
        ...
        int prevBestIndex = asMinArrayIndex(&m_scoresCalibTemp[0], &m_scoresCalibTemp[m_scoresCalibTemp.size() - 1]);
      ...
      case (Desc): {
        float actualBest = m_scoresCalib[0];
        int prevBestIndex = asMaxArrayIndex(&m_scoresCalibTemp[0], &m_scoresCalibTemp[m_scoresCalibTemp.size() - 1]);
        if (m_scoresCalibTemp[prevBestIndex] > actualBest) {
          wxLogMessage(_("Application of elitism after mutation."));
          // Randomly select a row to replace
          int randomRow = asRandom(0, m_scoresCalib.size() - 1, 1);
  ...
  switch (m_mutationsModeType) {
    ...
    case (NonUniform): {
      ...
      for (int iInd = 0; iInd < m_parameters.size(); iInd++) {
        // Mutate
        bool hasMutated = false;
        m_parameters[iInd].MutateNonUniform(mutationsProbability, m_generationNb, nbGenMax, minRate, hasMutated);
        if (hasMutated) m_scoresCalib[iInd] = NaNf;
