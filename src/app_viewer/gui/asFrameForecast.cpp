...
vroomDropFiles::vroomDropFiles(asFrameForecast *parent) : m_loaderFrame(parent) {
  wxASSERT(parent);
}
...
forecastDropFiles::forecastDropFiles(asFrameForecast *parent) : m_loaderFrame(parent) {
  wxASSERT(parent);
}
...
asFrameForecast::asFrameForecast(wxWindow *parent, wxWindowID id) : asFrameForecastVirtual(parent, id) {
  g_silentMode = false;
  ...
  m_toolBar->AddTool(asID_FRAME_GRID, wxT("Open analogs list"), *_img_frame_analogs, *_img_frame_analogs, wxITEM_NORMAL,
                     _("Open analogs list"), _("Open analogs list"), nullptr);
  ...
  // Forecasts
  m_panelSidebarForecasts =
      new asPanelSidebarForecasts(m_scrolledWindowOptions, m_forecastManager, wxID_ANY, wxDefaultPosition,
                                  wxDefaultSize, wxNO_BORDER | wxTAB_TRAVERSAL);
...
void asFrameForecast::UpdateLeadTimeSwitch() {
  ...
  m_leadTimeSwitcher = new asLeadTimeSwitcher(m_panelTop, &m_workspace, m_forecastManager, wxID_ANY, wxDefaultPosition,
                                              wxSize(width, height), wxTAB_TRAVERSAL);
...
void asFrameForecast::SwitchForecast(double increment) {
  ...
  // Get path
  wxString forecastsPath =
      m_forecastManager->GetFilePath(m_forecastViewer->GetMethodSelection(), m_forecastViewer->GetForecastSelection());
...
void asFrameForecast::UpdatePanelAnalogDates() {
  ...
  asResultsForecast *forecast =
      m_forecastManager->GetForecast(m_forecastViewer->GetMethodSelection(), m_forecastViewer->GetForecastSelection());
