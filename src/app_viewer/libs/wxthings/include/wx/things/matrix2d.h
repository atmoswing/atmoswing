...
public:
    wxThingsMatrix2D() {}
    wxThingsMatrix2D(const wxThingsMatrix2D &matrix, bool full_copy = false) { Create(matrix, full_copy); }
    wxThingsMatrix2D(int width, int height, bool zero = true)   { Create(width, height, zero); }
    wxThingsMatrix2D(int width, int height, const double *data) { Create(width, height, data); }
    wxThingsMatrix2D(int width, int height, double *data, bool static_data) { Create(width, height, data, static_data); }
    ...
    wxThingsMatrix2D AppendRows(const wxThingsMatrix2D& matrix) const { return InsertRows(-1, matrix); }
    // Append new cols to the matrix, matricies must have same number of rows
    wxThingsMatrix2D AppendCols(const wxThingsMatrix2D& matrix) const { return InsertCols(-1, matrix); }
    ...
    friend wxThingsMatrix2D operator+(wxThingsMatrix2D &a, wxThingsMatrix2D &b) { return a.Add(b); }
    friend wxThingsMatrix2D operator+(double scalar, wxThingsMatrix2D &b) { return b.Add(scalar); }
    friend wxThingsMatrix2D operator*(wxThingsMatrix2D &a, wxThingsMatrix2D &b) { return a.Mult(b); }
    friend wxThingsMatrix2D operator*(double scalar, wxThingsMatrix2D &a) { return a.Mult(scalar); }
    ...
    bool operator != (const wxThingsMatrix2D& matrix) { return !(*this == matrix); }
