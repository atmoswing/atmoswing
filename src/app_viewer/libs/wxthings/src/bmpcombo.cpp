...
BEGIN_EVENT_TABLE(wxBmpComboPopupChild, wxScrolledWindow)
                EVT_PAINT(wxBmpComboPopupChild::OnPaint) EVT_MOUSE_EVENTS(wxBmpComboPopupChild::OnMouse)
...
BEGIN_EVENT_TABLE(wxBmpComboLabel, wxWindow)
                EVT_PAINT(wxBmpComboLabel::OnPaint) EVT_CHAR(wxBmpComboLabel::OnChar) END_EVENT_TABLE()
...
BEGIN_EVENT_TABLE(wxBmpComboBox, DropDownBase)EVT_SIZE(wxBmpComboBox::OnSize) END_EVENT_TABLE()
...
{
    wxCHECK_RET(int(n
                        +count)<=GetCount(), wxT("invalid index"));
