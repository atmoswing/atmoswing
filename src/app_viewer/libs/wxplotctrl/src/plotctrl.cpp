...
wxString wxPlotCtrl_GetEventName(wxEventType eventType) {
  aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOTCTRL_ADD_CURVE) aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOTCTRL_DELETING_CURVE)
                                                          ...
                                                          aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOTCTRL_BEGIN_Y_LABEL_EDIT)
                                                              aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOTCTRL_END_Y_LABEL_EDIT)
                                                                  ...
                                                                  aDEFINE_LOCAL_EVENT_TYPE(
                                                                      wxEVT_PLOTCTRL_MOUSE_FUNC_CHANGING)
                                                                      aDEFINE_LOCAL_EVENT_TYPE(
                                                                          wxEVT_PLOTCTRL_MOUSE_FUNC_CHANGED)
...
EVT_MOUSE_EVENTS(wxPlotCtrlArea::OnMouse) EVT_CHAR(wxPlotCtrlArea::OnChar) EVT_KEY_DOWN(wxPlotCtrlArea::OnKeyDown)
...
EVT_MOUSE_EVENTS(wxPlotCtrlAxis::OnMouse) EVT_CHAR(wxPlotCtrlAxis::OnChar) END_EVENT_TABLE()
...
EVT_CHAR(wxPlotCtrl::OnChar) EVT_SCROLL(wxPlotCtrl::OnScroll) EVT_IDLE(wxPlotCtrl::OnIdle)
...
bool wxPlotCtrl::DoSelectRectangle(int curve_index, const wxRect2DDouble &rect, bool select, bool send_event) {
  ...
  if (plotData) {
    ...
    if (is_x_range && plotData->GetIsXOrdered()) {
      ...
      if ((plotData->GetXValue(min_) > xRange.m_min) && (min_ > 0) && (plotData->GetXValue(min_ - 1) > xRange.m_min))
        min_--;
...
void wxPlotCtrl::SetDefaultBoundingRect(const wxRect2DDouble &rect, bool send_event) {
  ...
  CalcBoundingPlotRect();
  SetZoom(m_areaClientRect.width / rect.m_width, m_areaClientRect.height / rect.m_height, rect.m_x, rect.m_y,
          send_event);
}
...
void wxPlotCtrl::ProcessAreaEVT_MOUSE_EVENTS(wxMouseEvent &event) {
  ...
  if (GetGreedyFocus() && (FindFocus() != m_area)) m_area->SetFocus();
  ...
  if (event.LeftDown() || event.LeftDClick()) {
    ...
    wxPlotCtrlEvent click_event(event.ButtonDClick() ? wxEVT_PLOTCTRL_DOUBLECLICKED : wxEVT_PLOTCTRL_CLICKED, GetId(),
                                this);
    ...
    if (FindCurve(wxPoint2DDouble(plotX, plotY), dpt, curve_index, data_index, &curvePt)) {
      ...
      if (plotCurve) {
        wxPlotCtrlEvent pt_click_event(
            event.ButtonDClick() ? wxEVT_PLOTCTRL_POINT_DOUBLECLICKED : wxEVT_PLOTCTRL_POINT_CLICKED, GetId(), this);
  ...
  // Finished marking rectangle or scrolling, perhaps
    ...
    if (m_area_mouse_func == wxPLOTCTRL_MOUSE_ZOOM) {
      if ((m_area_mouse_marker == wxPLOTCTRL_MARKER_RECT) && ((rightedRect.width > 10) && (rightedRect.height > 10)))
...
void wxPlotCtrl::OnScroll(wxScrollEvent &event) {
  ...
  if (event.GetId() == ID_PLOTCTRL_X_SCROLLBAR) {
    ...
    double origin_y =
        m_curveBoundingRect.GetTop() + m_curveBoundingRect.m_height * ((range - position - thumbsize) / range);
