    ...
    void OptimizeRedundant() {
        ...
        if (GetArgCount() == 1) {
            if (GetOp() == cMul || GetOp() == cAdd || GetOp() == cMin || GetOp() == cMax)
                if (!getp0().getsign()) ReplaceWith(*getp0());
        }
...
#define ConstantBinaryFun(token, fun)                                                              \
        ...
        Redo:
            for (pit a = GetBegin(); a != GetEnd(); ++a) {
                ...
                if (poslist.size() >= min) {
                    ...
                    for (j = poslist.begin(); j != poslist.end(); ++j) Erase(*j);
                    poslist.clear();
        ...
        if (GetOp() == cLog) {
            ...
            if (p->GetOp() == cPow) {
                ...
                if (adds.size()) {
                    ...
                    for (i = adds.begin(); i != adds.end(); ++i) tmp.AddParam(*i);
        ...
        if (GetOp() == cMul) {
            ...
            for (pit a = GetBegin(); a != GetEnd(); ++a) {
                ...
                if (poslist.size() >= 1) {
                    ...
                    for (i = poslist.begin(); i != poslist.end(); ++i) Erase(*i);
                    poslist.clear();
   ...
   public:
    ...
    void FinalOptimize() {
        ...
        for (pit a = GetBegin(); a != GetEnd(); ++a) (*a)->FinalOptimize();
    ...
    switch (GetOp()) {
        ...
        case cMul: {
            ...
            for (pcit a = GetBegin(); a != GetEnd(); ++a) {
                ...
                if (!done) pa->Assemble(byteCode, immed);
...
bool CodeTree::operator==(const CodeTree &b) const {
    ...
    if (IsVar())
        if (GetVar() != b.GetVar()) return false;
    if (data->IsFunc())
...
#define EAT(n, opcode)                                                                              \
    do {                                                                                            \
        ...
        for (unsigned a = 0, b = (n); a < b; ++a) stack[stacktop].AddParam(stack[newstacktop + a]); \
...
void FunctionParser::Optimize() {
    ...
    if ((data->ByteCodeSize = byteCode.size()) > 0) {
        ...
        for (unsigned a = 0; a < byteCode.size(); ++a) data->ByteCode[a] = byteCode[a];
    }
    ...
    if ((data->ImmedSize = immed.size()) > 0) {
        ...
        for (unsigned a = 0; a < immed.size(); ++a) data->Immed[a] = immed[a];
    }
}
