...
wxPlotPrintout::wxPlotPrintout(wxPlotCtrl* plotCtrl, const wxString& title)
    : wxPrintout(title),
      _plotCtrl(plotCtrl) {
    wxASSERT_MSG(_plotCtrl != NULL, wxT("NULL wxPlotCtrl for printing"));
}
...
bool wxPlotPrintout::OnPrintPage(int page_n) {
    wxDC* dc = GetDC();
    wxCHECK_MSG(dc && _plotCtrl, false, wxT("Invalid dc or plotctrl"));
    ...
    _plotCtrl->DrawWholePlot(dc, rect, dpi);
