...
bool asMethodForecasting::GetAnalogsDates(asResultsForecast &results, asParametersForecast &params, int iStep) {
  ...
  asTimeArray timeArrayArchive(timeStartArchive, timeEndArchive, params.GetAnalogsTimeStepHours(), asTimeArray::Simple);
    ...
    if (!params.NeedsPreprocessing(iStep, iPtor)) {
      ...
      for (int iPre = 0; iPre < params.GetPreprocessSize(iStep, iPtor); iPre++) {
        ...
        lastLeadTime = wxMin(lastLeadTime,
                             predictorRealtimePreprocess->GetForecastLeadTimeEnd() / 24.0 - params.GetTimeSpanDays());
  ...
  for (int iPtor = 0; iPtor < params.GetPredictorsNb(iStep); iPtor++) {
    ...
    if (!params.NeedsPreprocessing(iStep, iPtor)) {
      ...
      double ptorEndArchive = timeEndArchive + params.GetTimeShiftDays() + params.GetPredictorTimeAsDays(iStep, iPtor);
      ...
      double ptorEndTarget = timeEndTarget + params.GetTimeShiftDays() + params.GetPredictorTimeAsDays(iStep, iPtor);
      ...
      for (int iPre = 0; iPre < preprocessSize; iPre++) {
        ...
        asPredictor *predictorArchivePreprocess = asPredictor::GetInstance(
            ...
            params.GetPreprocessArchiveDataId(iStep, iPtor, iPre), m_batchForecasts->GetPredictorsArchiveDirectory());
      ...
      if (!PreprocessRealtimePredictors(m_storagePredictorsRealtimePreprocess, params.GetPreprocessMethod(iStep, iPtor),
                                        predictorRealtime)) {
        wxLogError(_("Data preprocessing failed."));
        wxDELETE(predictorArchive);
        wxDELETE(predictorRealtime);
