...
void asParametersOptimizationNelderMead::SetMeans(std::vector <asParametersOptimizationNelderMead> &vParameters, int elementsNb)
{
    ...
    for (int i_step=0; i_step<GetStepsNb(); i_step++)
    {
        ...
        for (int i_ptor=0; i_ptor<GetPredictorsNb(i_step); i_ptor++)
        {
            ...
            if(!m_StepsLocks[i_step].Predictors[i_ptor].TimeHours) SetPredictorTimeHours(i_step, i_ptor, 0);
    ...
    for (int i_set=0; i_set<elementsNb; i_set++)
    {
        ...
        for (int i_step=0; i_step<GetStepsNb(); i_step++)
        {
            ...
            for (int i_ptor=0; i_ptor<GetPredictorsNb(i_step); i_ptor++)
            {
                ...
                if(!m_StepsLocks[i_step].Predictors[i_ptor].TimeHours) SetPredictorTimeHours(i_step, i_ptor, GetPredictorTimeHours(i_step, i_ptor) + vParameters[i_set].GetPredictorTimeHours(i_step, i_ptor));
    ...
    for (int i_step=0; i_step<GetStepsNb(); i_step++)
    {
        ...
        for (int i_ptor=0; i_ptor<GetPredictorsNb(i_step); i_ptor++)
        {
            ...
            if(!m_StepsLocks[i_step].Predictors[i_ptor].TimeHours) SetPredictorTimeHours(i_step, i_ptor, GetPredictorTimeHours(i_step, i_ptor) / (double)elementsNb);
...
void asParametersOptimizationNelderMead::GeometricTransform(asParametersOptimizationNelderMead &refParams, float coefficient)
{
    ...
    for (int i_step=0; i_step<GetStepsNb(); i_step++)
    {
        ...
        for (int i_ptor=0; i_ptor<GetPredictorsNb(i_step); i_ptor++)
        {
            ...
            if(!m_StepsLocks[i_step].Predictors[i_ptor].TimeHours) SetPredictorTimeHours(i_step, i_ptor, refParams.GetPredictorTimeHours(i_step, i_ptor)+coefficient*(refParams.GetPredictorTimeHours(i_step, i_ptor)-GetPredictorTimeHours(i_step,i_ptor)));
...
void asParametersOptimizationNelderMead::Reduction(asParametersOptimizationNelderMead &refParams, float sigma)
{
    ...
    for (int i_step=0; i_step<GetStepsNb(); i_step++)
    {
        ...
        for (int i_ptor=0; i_ptor<GetPredictorsNb(i_step); i_ptor++)
        {
            ...
            if(!m_StepsLocks[i_step].Predictors[i_ptor].TimeHours) SetPredictorTimeHours(i_step, i_ptor, refParams.GetPredictorTimeHours(i_step, i_ptor)+sigma*(GetPredictorTimeHours(i_step, i_ptor)-refParams.GetPredictorTimeHours(i_step, i_ptor)));
