...
bool asProcessor::GetAnalogsDates(std::vector<asPredictor *> predictorsArchive,
  ...
  switch (method) {
    ...
    case (asCUDA): {
      ...
      datesArchiveSlt.Init(timeTargetSelection[0], params->GetAnalogsIntervalDays(), params->GetAnalogsExcludeDays());
      ...
      for (int i = 0; i < timeTargetSelectionSize + nStreams / 2; i++) {
        ...
        if (i < timeTargetSelectionSize) {
          ...
          for (int iDateArch = 0; iDateArch < datesArchiveSlt.GetSize(); iDateArch++) {
            ...
            if (timeArchData.size() > iTimeArchStart + 1 &&
              ...
              iTimeArchRelative = asFind(&timeArchData[iTimeArchStart], &timeArchData[timeArchData.size() - 1],
          ...
          asProcessorCuda::ProcessCriteria(dData, ptorStart, iTimeTarg, dIdxArch, dRes, nbCand, colsNb, rowsNb, weights,
                                           crit, streams[streamId], offset);
        ...
        if (i >= nStreams / 2) {
          ...
          checkCudaErrors(cudaMemcpyAsync(&hRes[offset], &dRes[offset], nbCandidates[streamId] * sizeof(float),
          ...
          for (int iDateArch = 0; iDateArch < nbCandidates[streamId]; iDateArch++) {
            ...
            InsertInArrays(isAsc, analogsNb, currentDates[offset + iDateArch], hRes[offset + iDateArch], resCounter,
          ...
          if (resCounter < analogsNb) {
            ...
            wxLogWarning(_("Analogs number (%d) > resCounter (%d), date array size (%d) with "
                           "%d days intervals."),
                         analogsNb, resCounter, datesArchiveSlt.GetSize(), params->GetAnalogsIntervalDays());
          }
      ...
      for (int iThread = 0; iThread < threadsNb; iThread++) {
        ...
        *success = false;
        ...
        auto *thread = new asThreadGetAnalogsDates(
            ...
            &timeArrayTargetData, &timeArrayTargetSelection, criteria, params, step, vRowsNb, vColsNb, start, end,
            &finalAnalogsCriteria, &finalAnalogsDates, flag, allowDuplicateDates, success);
    ...
    case (asSTANDARD): {
      ...
      asTimeArray dateArrayArchiveSelection(timeArrayArchiveSelection.GetStart(), timeArrayArchiveSelection.GetEnd(),
                                            params->GetAnalogsTimeStepHours(), params->GetTimeArrayAnalogsMode());
      if (timeArrayArchiveSelection.HasForbiddenYears()) {
      ...
      for (int iDateTarg = 0; iDateTarg < timeTargetSelectionSize; iDateTarg++) {
        ...
        for (int iMem = 0; iMem < membersNb; ++iMem) {
          ...
          for (int iDateArch = 0; iDateArch < dateArrayArchiveSelection.GetSize(); iDateArch++) {
            int iTimeArchRelative = FindNextDate(dateArrayArchiveSelection, timeArchiveData, iTimeArchStart, iDateArch);
            ...
            for (int iPtor = 0; iPtor < predictorsNb; iPtor++) {
              ...
              float tmpScore =
                  criteria[iPtor]->Assess(*vTargData[iPtor], *vArchData[iPtor], vRowsNb[iPtor], vColsNb[iPtor]);
              ...
              if (asIsNaN(tmpScore)) {
                ...
                wxLogWarning(_("NaNs were found in the criteria values (%s/%s)."),
                             predictorsArchive[iPtor]->GetProduct(), predictorsArchive[iPtor]->GetDataId());
                wxLogWarning(_("Target date: %s, archive date: %s."),
                             asTime::GetStringTime(timeTargetSelection[iDateTarg]),
                             asTime::GetStringTime(dateArrayArchiveSelection[iDateArch]));
              }
            ...
            if (!allowDuplicateDates && iMem > 0) {
              ...
              InsertInArrays(isAsc, analogsNb, (float)timeArchiveData[iTimeArch], thisScore, counter, scoreArrayOneDay,
                             dateArrayOneDay);
            }
        ...
        if (counter < analogsNb) {
          wxLogWarning(
              _("There is not enough available data to satisfy the number of analogs "
                "(in asProcessor::GetAnalogsDates)."));
        }
...
bool asProcessor::GetAnalogsSubDates(std::vector<asPredictor *> predictorsArchive,
  ...
  if (analogsNb > analogsNbPrevious) {
    wxLogError(_("The given analog number (%d) is superior to the previous step (%d)."), analogsNb, analogsNbPrevious);
    return false;
  }
  ...
  switch (method) {
    ...
    case (asCUDA): {
      ...
      for (int i = 0; i < timeTargetSelectionSize + nStreams / 2; i++) {
        ...
        if (i < timeTargetSelectionSize) {
          ...
          int iTimeTarg =
              asFind(&timeTargetData[0], &timeTargetData[timeTargetDataSize - 1], timeTargetSelection[i], 0.01);
          wxASSERT_MSG(
              iTimeTarg >= 0,
    ...
    case (asMULTITHREADS): {
      ...
      for (int iThread = 0; iThread < threadsNb; iThread++) {
        ...
        *success = false;
        ...
        wxASSERT_MSG(end >= start, wxString::Format("start = %d, end = %d, timeTargetSelectionSize = %d", start, end,
                                                    timeTargetSelectionSize));

        asThreadGetAnalogsSubDates *thread = new asThreadGetAnalogsSubDates(
            predictorsArchive, predictorsTarget, &timeArrayArchiveData, &timeArrayTargetData, &timeTargetSelection,
            criteria, params, step, vTargData, vArchData, vRowsNb, vColsNb, start, end, &finalAnalogsCriteria,
            &finalAnalogsDates, &analogsDates, flag, success);
        threadType = thread->GetType();
    ...
    case (asSTANDARD): {
      ...
      for (int iAnalogDate = 0; iAnalogDate < timeTargetSelectionSize; iAnalogDate++) {
        int iTimeTarg =
            asFind(&timeTargetData[0], &timeTargetData[timeTargetDataSize - 1], timeTargetSelection[iAnalogDate], 0.01);
        wxASSERT_MSG(
            iTimeTarg >= 0,
            wxString::Format(_("Looking for %s in betwwen %s and %s."),
            ...
            if (iTimeArch != asNOT_FOUND && iTimeArch != asOUT_OF_RANGE) {
              ...
              for (int iPtor = 0; iPtor < predictorsNb; iPtor++) {
                ...
                wxASSERT_MSG(
                    ...
                    wxString::Format("%s (%d th element) in archive, %s (%d th element) in target: vArchData size = "
                                     "%d, vTargData size = %d",
              ...
              if (counter > analogsNb - 1) {
                if (isAsc) {
                  if (thisscore < scoreArrayOneDay[analogsNb - 1]) {
                    asArraysInsert(&scoreArrayOneDay[0], &scoreArrayOneDay[analogsNb - 1], &dateArrayOneDay[0],
                                   &dateArrayOneDay[analogsNb - 1], Asc, thisscore, (float)timeArchiveData[iTimeArch]);
                  }
                } else {
                  if (thisscore > scoreArrayOneDay[analogsNb - 1]) {
                    asArraysInsert(&scoreArrayOneDay[0], &scoreArrayOneDay[analogsNb - 1], &dateArrayOneDay[0],
                                   &dateArrayOneDay[analogsNb - 1], Desc, thisscore, (float)timeArchiveData[iTimeArch]);
                  }
                }
              } else if (counter < analogsNb - 1) {
                // Add score and date to the vectors
        ...
        if (counter < analogsNb) {
          wxLogWarning(
              _("There is not enough available data to satisfy the number of analogs (in "
                "asProcessor::GetAnalogsSubDates)."));
...
bool asProcessor::GetAnalogsValues(asPredictand &predictand, asResultsDates &anaDates, asParameters *params,
  ...
  if (indexTargDatesStart == asNOT_FOUND || indexTargDatesStart == asOUT_OF_RANGE || indexTargDatesEnd == asNOT_FOUND ||
      indexTargDatesEnd == asOUT_OF_RANGE) {
      ...
      if (!asIsNaN(currentAnalogDate)) {
        ...
        if ((currentAnalogDate >= timeStart) && (currentAnalogDate <= timeEnd)) {
          ...
          wxLogError(_("The current analog date (%s) is outside of the allowed period (%s-%s))."),
                     ...
                     asTime::GetStringTime(timeStart, "DD.MM.YYYY"), asTime::GetStringTime(timeEnd, "DD.MM.YYYY"));
          for (int iStat = 0; iStat < (int)stations.size(); iStat++) {
            finalAnalogValuesNorm[iStat](iTargetDatenew, iAnalogDate) = NaNf;
            finalAnalogValuesRaw[iStat](iTargetDatenew, iAnalogDate) = NaNf;
          }
...
#ifndef UNIT_TESTING
#ifdef _DEBUG
    wxASSERT(!asHasNaN(&finalAnalogCriteria(iTargetDatenew, 0), &finalAnalogCriteria(iTargetDatenew, analogsNb - 1)));
#endif
#endif
  }
