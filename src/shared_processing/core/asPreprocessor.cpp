  ...
  for (int iTime = 0; iTime < timeSize; iTime++) {
    for (int iMem = 0; iMem < membersNb; iMem++) {
      for (int iRow = 0; iRow < rowsNb; iRow++) {
        for (int iCol = 0; iCol < colsNb; iCol++) {
          wind = (float)std::sqrt(
              predictors[0]->GetData()[iTime][iMem](iRow, iCol) * predictors[0]->GetData()[iTime][iMem](iRow, iCol) +
              predictors[1]->GetData()[iTime][iMem](iRow, iCol) * predictors[1]->GetData()[iTime][iMem](iRow, iCol));
