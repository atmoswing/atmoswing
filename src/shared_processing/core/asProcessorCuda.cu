...
bool asProcessorCuda::ProcessCriteria(const float *dData, std::vector<long> ptorStart, int indexTarg,
  ...
  for (int iPtor = 0; iPtor < ptorStart.size(); iPtor++) {
    ...
    switch (criteria[iPtor]) {
      ...
      case S1grads:
      case S2grads:
        processS1grads<<<blocksNb3D, blockSize, 0, stream>>>(dData, ptorStart[iPtor], nbCandidates, ptsNb, indexTarg,
      ...
      case MD:
        ...
        break;
      case RMSE:
        processRMSE<<<blocksNb3D, blockSize, 0, stream>>>(dData, ptorStart[iPtor], nbCandidates, ptsNb, indexTarg,
                                                          indicesArch, weights[iPtor], dRes, offset);
        break;
      case RSE:
        processRSE<<<blocksNb3D, blockSize, 0, stream>>>(dData, ptorStart[iPtor], nbCandidates, ptsNb, indexTarg,
                                                         indicesArch, weights[iPtor], dRes, offset);
        break;
      case SAD:
        processSAD<<<blocksNb3D, blockSize, 0, stream>>>(dData, ptorStart[iPtor], nbCandidates, ptsNb, indexTarg,
                                                         indicesArch, weights[iPtor], dRes, offset);
        break;
      case DMV:
        processDMV<<<blocksNb3D, blockSize, 0, stream>>>(dData, ptorStart[iPtor], nbCandidates, ptsNb, indexTarg,
                                                         indicesArch, weights[iPtor], dRes, offset);
        break;
      case DSD:
        processDSD<<<blocksNb3D, blockSize, 0, stream>>>(dData, ptorStart[iPtor], nbCandidates, ptsNb, indexTarg,
                                                         indicesArch, weights[iPtor], dRes, offset);
        break;
      default:
