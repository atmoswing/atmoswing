...
asPredictorNoaaOisst2::asPredictorNoaaOisst2(const wxString& dataId)
    ...
    _datasetId = "NOAA_OISST_v2";
    _provider = "NOAA";
    _datasetName = "Optimum Interpolation Sea Surface Temperature, version 2";
    _fileType = asFile::Netcdf;
    _strideAllowed = true;
    _nanValues.push_back(32767);
    _nanValues.push_back(936 * std::pow(10.f, 34.f));
    _fileNamePattern = "%d/AVHRR/sst4-path-eot.%4d%02d%02d.nc";
...
void asPredictorNoaaOisst2::ListFiles(asTimeArray& timeArray) {
    for (double date = timeArray.GetFirst(); date <= timeArray.GetLast(); date++) {
        ...
        _files.push_back(GetFullDirectoryPath() + asStrF(_fileNamePattern, asTime::GetYear(date), asTime::GetYear(date),
