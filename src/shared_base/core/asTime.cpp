...
double asTime::NowMJD(int timezone) {
    ...
    switch (timezone) {
        ...
        case asLOCAL:
            ...
            break;
        default:
            asThrow(_("The timezone is not correctly set"));
    }
...
double asTime::GetTimeFromString(const wxString& datestr, TimeFormat format) {
    ...
    switch (format) {
        case (ISOdate):
            ...
            if (datestr.Len() == 10) {
                if (!datestr.Mid(0, 4).ToLong(&year)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(5, 2).ToLong(&month)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(8, 2).ToLong(&day)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(year, month, day);
            }

            asThrow(asStrF(errormsglength, (int)datestr.Len(), 14, 19));
        ...
        case (ISOdateTime):
            ...
            if (datestr.Len() == 19) {
                ...
                if (!datestr.Mid(5, 2).ToLong(&month)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(8, 2).ToLong(&day)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(11, 2).ToLong(&hour)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(14, 2).ToLong(&min)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(17, 2).ToLong(&sec)) asThrow(asStrF(errormsgconversion, datestr));
        ...
        case (DD_MM_YYYY):
            ...
            if (datestr.Len() == 10) {
                if (!datestr.Mid(0, 2).ToLong(&day)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(3, 2).ToLong(&month)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(6, 4).ToLong(&year)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(year, month, day);
            } else if (datestr.Len() == 8) {
                if (!datestr.Mid(0, 2).ToLong(&day)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(2, 2).ToLong(&month)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(4, 4).ToLong(&year)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(year, month, day);
            }

            asThrow(asStrF(errormsglength, (int)datestr.Len(), 8, 10));
        ...
        case (YYYYMMDD):
            ...
            if (datestr.Len() == 10) {
                ...
                if (!datestr.Mid(0, 4).ToLong(&year)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(4, 2).ToLong(&month)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(6, 2).ToLong(&day)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(year, month, day);
            }
        ...
        case (YYYY_MM_DD_hh):
            ...
            if (datestr.Len() == 13) {
                ...
                if (!datestr.Mid(4, 2).ToLong(&month)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(6, 2).ToLong(&day)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(8, 2).ToLong(&hour)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(year, month, day, hour);
            }

            asThrow(asStrF(errormsglength, (int)datestr.Len(), 10, 13));
        ...
        case (DD_MM_YYYY_hh_mm):
            ...
            if (datestr.Len() == 16) {
                ...
                if (!datestr.Mid(4, 4).ToLong(&year)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(8, 2).ToLong(&hour)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(10, 2).ToLong(&min)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(year, month, day, hour, min);
            }

            asThrow(asStrF(errormsglength, (int)datestr.Len(), 12, 16));
        ...
        case (DD_MM_YYYY_hh_mm_ss):
            ...
            if (datestr.Len() == 19) {
                ...
                if (!datestr.Mid(8, 2).ToLong(&hour)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(10, 2).ToLong(&min)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(12, 2).ToLong(&sec)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(year, month, day, hour, min, sec);
            }
        ...
        case (hh_mm):
            ...
            if (datestr.Len() == 5) {
                if (!datestr.Mid(0, 2).ToLong(&hour)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(3, 2).ToLong(&min)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(0, 0, 0, hour, min);
            } else if (datestr.Len() == 4) {
                if (!datestr.Mid(0, 2).ToLong(&hour)) asThrow(asStrF(errormsgconversion, datestr));
                if (!datestr.Mid(2, 2).ToLong(&min)) asThrow(asStrF(errormsgconversion, datestr));
                return GetMJD(0, 0, 0, hour, min);
            }

            asThrow(asStrF(errormsglength, (int)datestr.Len(), 4, 5));

        case (guess):

            if (datestr.Mid(0, 1) == "+") {
                if (!datestr.Mid(1).ToLong(&hour)) asThrow(asStrF(errormsgconversion, datestr));
                date = NowMJD(asUTM);
                date += (double)hour / 24;
                return date;
                ...
                if (datestr.Len() == 10) {
                    ...
                    asThrow(asStrF(errormsgconversion, datestr));
            ...
            break;
        default:
            asThrow(_("The date format is not correctly set"));
    }
}
