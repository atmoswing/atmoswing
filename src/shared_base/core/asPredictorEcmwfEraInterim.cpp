...
asPredictorEcmwfEraInterim::asPredictorEcmwfEraInterim(const wxString& dataId)
    ...
    _datasetId = "ECMWF_ERA_interim";
    _provider = "ECMWF";
    _datasetName = "ERA-interim";
...
bool asPredictorEcmwfEraInterim::Init() {
    ...
    if (IsPressureLevel()) {
        ...
        if (_dataId.IsSameAs("d", false)) {
            ...
            _fileVarName = "d";
            _unit = per_s;
        } else if (_dataId.IsSameAs("mont", false)) {
            _parameter = MontgomeryPotential;
            _parameterName = "Montgomery potential";
            _fileVarName = "mont";
            _unit = m2_s2;
        } else if (IsPressure()) {
            _parameter = Pressure;
    ...
    } else if (IsSurfaceLevel() || _product.IsSameAs("sfa", false) || _product.IsSameAs("sfan", false) ||
               _product.IsSameAs("sff", false) || _product.IsSameAs("sffc", false)) {
        ...
        if (IsPressure()) {
            ...
            _fileVarName = "pres";
            _unit = Pa;
        } else if (_dataId.IsSameAs("pt", false)) {
...
void asPredictorEcmwfEraInterim::ListFiles(asTimeArray& timeArray) {
    ...
    for (size_t i = 0; i < listFiles.Count(); ++i) {
        ...
        if (fileYear < firstYear || fileYear > lastYear) {
            if (_product.IsSameAs("sff", false) || _product.IsSameAs("sffc", false)) {
