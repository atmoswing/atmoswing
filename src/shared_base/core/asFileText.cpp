...
void asFileText::AddContent(const wxString& lineContent) {
    ...
    if (m_file.fail())
        throw exception(asStrF(_("An error occured while trying to write in file %s"), m_fileName.GetFullPath()));
}
...
wxString asFileText::GetNextLine() {
    ...
    if (!m_file.eof()) {
        ...
        throw exception(asStrF(_("You are trying to read a line after the end of the file %s"), m_fileName.GetFullPath()));
    }
