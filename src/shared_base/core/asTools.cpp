...
int asTools::SortedArraySearchT(T* pArrStart, T* pArrEnd, T targetvalue, T tolerance, int showWarning)
{
    ...
    if (*pLast>*pFirst)
    {
        ...
        if (abs(targetvalue-*pLast)<=abs(targetvalue-*(pLast+1)))
        {
            if(abs(targetvalue-*pLast)<=tolerance)
            {
                ...
                {
                    if (asSHOW_WARNINGS) asLogWarning(_("The value is out of the array range."));
    ...
    else
    {
        if (pLast-pFirst==0)
        {
            if (asSHOW_WARNINGS) asLogWarning(_("There is only one value in the array."));
    ...
    if (asSHOW_WARNINGS) asLogWarning(_("The value was not found in the array."));
    return asNOT_FOUND;
}
...
bool asTools::SortArraysT(T* pArrRefStart, T* pArrRefEnd, T* pArrOtherStart, T* pArrOtherEnd, Order order)
{
    ...
    if (vlength>0 && vlength==ovlength)
    {
        ...
        asLogError(_("The has a negative size..."));
        return false;
    }
    return true;
}
