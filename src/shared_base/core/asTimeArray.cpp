...
bool asTimeArray::Init(asDataPredictand &predictand, const wxString &serieName, int stationId, float minThreshold, float maxThreshold)
{
    m_TimeArray.resize(0);
...
bool asTimeArray::Init(double forecastdate, double intervaldays, double exclusiondays, asDataPredictand &predictand, const wxString &serieName, int stationId, float minThreshold, float maxThreshold)
{
    wxASSERT(exclusiondays>0);
...
bool asTimeArray::BuildArraySimple()
{
    ...
    wxASSERT_MSG(fmod ((m_End-m_Start), m_TimeStepDays)==0, wxString::Format("start=%f, end=%f, timestepdays=%f", m_Start, m_End, m_TimeStepDays ));
...
bool asTimeArray::BuildArraySeasons(double forecastdate)
{
    ...
    for (int i_year=startfirstyearstruct.year; i_year<=endlastyearstruct.year; i_year++)
    {
        ...
        {
            thistimestep = GetMJD(i_year, GetSeasonStart(forecastseason).month, GetSeasonStart(forecastseason).day, startfirstyearstruct.hour, startfirstyearstruct.min);
...
bool asTimeArray::BuildArrayPredictandThresholds(asDataPredictand &predictand, const wxString &serieName, int stationId, float minThreshold, float maxThreshold)
{
    ...
    for(int i=0; i<predictandTimeArray.size(); i++)
    {
        ...
        int rowTimeArray = asTools::SortedArraySearchFloor(&fullTimeArray[0], &fullTimeArray[fullTimeArray.size()-1], predictandTimeArray[i]);
