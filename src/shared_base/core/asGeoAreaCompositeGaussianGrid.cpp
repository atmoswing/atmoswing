        ...
        : asGeoAreaCompositeGrid(Level, Height)
{
    ...
    if (!IsOnGrid(Xmin, Ymin))
        asThrowException(
                wxString::Format(_("The given area does not match a gaussian grid (Xmin = %g, Ymin = %g)."), Xmin,
                                 Ymin));
    ...
    if (Xptsnb < 0)
        asThrowException(
                wxString::Format(_("The given width (points number) is not consistent in the gaussian grid: %d"),
                                 Xptsnb));
    if (Yptsnb < 0)
        asThrowException(
                wxString::Format(_("The given height (points number) is not consistent in the gaussian grid: %d"),
                                 Yptsnb));
...
void asGeoAreaCompositeGaussianGrid::BuildGaussianGrid()
{
    switch (m_gaussianGridType) {
        case (asGeoAreaGaussianGrid::T62):
            ...
            m_fullAxisX = Eigen::ArrayXd::LinSpaced(577, -360, 720); // Normally: LinSpaced(192, 0, 358.125) but extended to accept negative longitudes.
            break;
        default:
            asThrowException(_("The Gaussian grid type was not correctly defined. T62 taken as default."));
    }
}
...
Array1DDouble asGeoAreaCompositeGaussianGrid::GetYaxisComposite(int compositeNb) const
{
...
int asGeoAreaCompositeGaussianGrid::GetXaxisCompositePtsnb(int compositeNb) const
{
    double Xmin = GetComposite(compositeNb).GetXmin();
    double Xmax = GetComposite(compositeNb).GetXmax();
...
int asGeoAreaCompositeGaussianGrid::GetYaxisCompositePtsnb(int compositeNb) const
{
    double Ymin = GetComposite(compositeNb).GetYmin();
    double Ymax = GetComposite(compositeNb).GetYmax();
