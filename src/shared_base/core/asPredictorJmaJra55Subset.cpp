    ...
    for (int iYear = timeArray.GetStartingYear(); iYear <= timeArray.GetEndingYear(); iYear++) {
        ...
        if (m_monthlyFiles) {
            for (int iMonth = firstMonth; iMonth <= lastMonth; ++iMonth) {
                ...
                if (nbFiles == 0) {
                    asThrowException(asStrF(_("No JRA-55 file found for this pattern : %s."), filePattern));
                } else if (nbFiles > 1) {
                    asThrowException(asStrF(_("Multiple JRA-55 files found for this pattern : %s."), filePattern));
                }
