...
short asFileNetcdf::GetAttShort(const wxString &attName, const wxString &varName) {
  ...
  if (varName.IsEmpty()) {  // Global attribute
    ...
    if (nctype == NC_INT) {
      ...
      asThrowException(
          wxString::Format(_("The attribute (%s.%s) type (%d) in file doesn't match the desired type (%d)."), varName,
                           attName, (int)nctype, (int)NC_SHORT));
    }
...
void asFileNetcdf::GetVarArray(const wxString &varName, const size_t indexStart[], const size_t indexCount[],
  ...
  if (nctype != NC_FLOAT && nctype != NC_SHORT && nctype != NC_DOUBLE && nctype != NC_INT && nctype != NC_INT64) {
    asThrowException(wxString::Format(
        _("The variable (%s) type (%d) in file doesn't match the desired type (numeric)."), varName, (int)nctype));
  }
    ...
    for (auto &att : var.atts) {
      if (att.pValue != nullptr) {
        ...
        switch (nctype) {
          ...
          default:
            asThrowException(
                wxString::Format(_("NetCDF file: data type (%d) of attribute %s not taken into account in AtmoSwing."),
                                 nctype, att.name));
            break;
        }
  ...
  for (auto &att : m_struct.atts) {
    if (att.pValue != nullptr) {
      ...
      switch (nctype) {
        ...
        default:
          asThrowException(wxString::Format(
              _("NetCDF file: data type (%d) of attribute %s not taken into account in AtmoSwing."), nctype, att.name));
          break;
      }
  ...
  for (int attId = 0; attId < nAtts; attId++) {
    ...
    if (len > 1) {
      ...
      switch (nctype) {
        ...
        default:
          wxLogError(_("NetCDF file: data type (%d) of attribute %s not taken into account in AtmoSwing."), nctype,
                     m_struct.atts[attId].name);
          return false;
          break;
      }
    ...
    for (int attId = 0; attId < nAttsVar; attId++) {
      ...
      if (len > 1) {
        ...
        switch (nctype) {
          ...
          default:
            wxLogError(_("NetCDF file: data type (%d) of attribute %s not taken into account in AtmoSwing."), nctype,
                       m_struct.vars[varId].atts[attId].name);
            return false;
            break;
        }
