...
bool asPredictor::Load(asAreaCompGrid *desiredArea, asTimeArray &timeArray, float level) {
  ...
  try {
    ...
    if (m_time.size() > m_data.size()) {
      wxLogError(_("The date and the data array lengths do not match (time = %d and data = %d)."), (int)m_time.size(),
                 (int)m_data.size());
...
bool asPredictor::GetDataFromFile(asFileNetcdf &ncFile, vvva2f &compositeData) {
  ...
  for (int iArea = 0; iArea < compositeData.size(); iArea++) {
    ...
    ncFile.GetVarSample(m_fileVarName, GetIndexesStartNcdf(iArea), GetIndexesCountNcdf(iArea), GetIndexesStrideNcdf(),
                        &dataF[indexBeginning]);
  ...
  if (yStartIndexReal == 0 && xStartIndex == 0 && yLength == m_axisLat.size() && xLength == m_axisLon.size()) {
    ...
    if (IsPreprocessed()) {
      ...
      if (method.IsSameAs("Gradients") || method.IsSameAs("SimpleGradients") || method.IsSameAs("RealGradients") ||
          method.IsSameAs("SimpleGradientsWithGaussianWeights") ||
        ...
        */
        ...
        for (int i = 0; i < originalData.size(); i++) {
          for (int j = 0; j < originalData[i].size(); j++) {
            ...
            a2f dat2 = originalData[i][j].block(yStartIndexReal + m_axisLat.size(), xStartIndex, yLength, xLength - 1);
        ...
        for (int i = 0; i < originalData.size(); i++) {
          for (int j = 0; j < originalData[i].size(); j++) {
            ...
            a2f dat2 = originalData[i][j].block(yStartIndexReal + m_axisLat.size(), xStartIndex, yLength, xLength);
        ...
        return true;
                 ...
                 method.IsSameAs("HumidityIndex") || method.IsSameAs("Addition") || method.IsSameAs("Average")) {
        vva2f originalData = m_data;
...
bool asPredictor::InterpolateOnGrid(asAreaCompGrid *dataArea, asAreaCompGrid *desiredArea) {
  ...
  if (dataArea->GetNbComposites() > 1 &&
      dataArea->GetComposite(1).GetXmin() != desiredArea->GetComposite(1).GetXmin()) {
  ...
  if (dataArea->GetNbComposites() > 1 &&
      dataArea->GetComposite(1).GetYmin() != desiredArea->GetComposite(1).GetYmin()) {
    changeYstart = true;
  }
    ...
    for (int iTime = 0; iTime < m_data.size(); iTime++) {
      for (int iMem = 0; iMem < m_data[0].size(); iMem++) {
        ...
        for (int iLat = 0; iLat < finalLengthLat; iLat++) {
          ...
          } else if (axisDataLat.size() > indexLastLat + 2 && axisDataLat[indexLastLat + 2] == axisFinalLat[iLat]) {
            indexYfloor = indexLastLat + 2;
            indexYceil = indexLastLat + 2;
          } else {
            // Search for floor and ceil
            indexYfloor = indexLastLat +
                          asFindFloor(&axisDataLat[indexLastLat], &axisDataLat[axisDataLatEnd], axisFinalLat[iLat]);
            indexYceil =
                indexLastLat + asFindCeil(&axisDataLat[indexLastLat], &axisDataLat[axisDataLatEnd], axisFinalLat[iLat]);
          }
          ...
          if (indexYfloor == asOUT_OF_RANGE || indexYfloor == asNOT_FOUND || indexYceil == asOUT_OF_RANGE ||
            ...
            wxLogError(_("The desired point is not available in the data for interpolation. Latitude %f "
                       ...
                       axisFinalLat[iLat], axisDataLat[indexLastLat], indexLastLat, axisDataLat[axisDataLatEnd],
                       axisDataLatEnd, (int)axisDataLat.size());
            return false;
          }
          wxASSERT_MSG(indexYfloor >= 0, wxString::Format("%f in %f to %f", axisFinalLat[iLat],
          ...
          for (int iLon = 0; iLon < finalLengthLon; iLon++) {
            ...
            if (axisDataLon.size() > indexLastLon + 1 && axisDataLon[indexLastLon + 1] == axisFinalLon[iLon]) {
              indexXfloor = indexLastLon + 1;
              indexXceil = indexLastLon + 1;
            } else if (axisDataLon.size() > indexLastLon + 2 && axisDataLon[indexLastLon + 2] == axisFinalLon[iLon]) {
              indexXfloor = indexLastLon + 2;
              indexXceil = indexLastLon + 2;
            } else {
              // Search for floor and ceil
              indexXfloor = indexLastLon +
                            asFindFloor(&axisDataLon[indexLastLon], &axisDataLon[axisDataLonEnd], axisFinalLon[iLon]);
              indexXceil = indexLastLon +
                           asFindCeil(&axisDataLon[indexLastLon], &axisDataLon[axisDataLonEnd], axisFinalLon[iLon]);
            }
            ...
            if (dX == 0 && dY == 0) {
              ...
              latlonTimeData[iTime][iMem](iLat, iLon) = (1 - dX) * (1 - dY) * valLLcorner + (1 - dX) * (dY)*valULcorner;
            } else if (dY == 0) {
              valLLcorner = m_data[iTime][iMem](indexYfloor, indexXfloor);
              ...
              latlonTimeData[iTime][iMem](iLat, iLon) = (1 - dX) * (1 - dY) * valLLcorner +
                                                        (1 - dX) * (dY)*valULcorner + (dX) * (1 - dY) * valLRcorner +
                                                        (dX) * (dY)*valURcorner;
            }
          }
...
bool asPredictor::IsPressure() const {
  return m_dataId.IsSameAs("pressure", false) || m_dataId.IsSameAs("press", false) || m_dataId.IsSameAs("pres", false);
}
