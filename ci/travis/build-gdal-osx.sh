...
if [ ! "$(ls -A ${HOME}/.libs/include/gdal.h)" ] || [ "$REBUILD_GDAL" = true ]; then
  ...
  tar -xzf gdal.tar.gz
  cd gdal-2.4.1
  ./configure --prefix=${HOME}/.libs --with-proj=${HOME}/.libs --with-sqlite3=no --with-python=no --with-pg=no --with-grass=no --with-jasper=/usr --with-curl=/usr --with-jpeg=internal --with-png=internal --silent
  make -j6 > /dev/null
  make install
  cd ..
