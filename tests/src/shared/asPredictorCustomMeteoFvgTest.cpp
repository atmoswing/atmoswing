...
TEST(PredictorCustomMeteoFvg, GetCorrectSynopticPredictors) {
    ...
    predictor = asPredictor::GetInstance("Custom_MeteoFVG_synop_packed", "vertdiff/MB850500", ".");
    ASSERT_TRUE(predictor->GetParameter() == asPredictor::MaximumBuoyancy);
    ASSERT_TRUE(predictor->GetDataId().IsSameAs("MB850500"));
    wxDELETE(predictor);
}
...
TEST(PredictorCustomMeteoFvg, LoadFullTimeArrayMeso) {
    asTimeArray dates(asTime::GetMJD(2011, 7, 18, 6), asTime::GetMJD(2011, 7, 19, 24), 6, "Simple");
    dates.Init();
...
TEST(PredictorCustomMeteoFvg, LoadMesoPacked) {
    asTimeArray dates(asTime::GetMJD(2015, 5, 26, 6), asTime::GetMJD(2015, 5, 30, 24), 6, "Simple");
    dates.Init();
